...
function Tooth() {
  ...
  useFrame((state) => {
    ...
    const floatY = Math.sin(state.clock.elapsedTime * 0.4) * 0.04;
    groupRef.current.position.y = floatY;
  });

  const crownGeo = useMemo(() => {
    const g = new THREE.SphereGeometry(0.65, 48, 48);
    g.scale(0.85, 0.8, 0.65);
    return g;
  }, []);

  const neckGeo = useMemo(() => new THREE.CylinderGeometry(0.38, 0.42, 0.32, 32), []);
  const rootGeo = useMemo(() => new THREE.CylinderGeometry(0.28, 0.06, 0.55, 32), []);
  ...
  return (
    <group ref={groupRef} position={[1.4, 0, 0]}>
      ...
      <mesh position={[0, -0.46, 0]} geometry={neckGeo}>
        <meshPhysicalMaterial
          color="#e8e0d8"
          roughness={0.15}
          metalness={0.05}
      ...
      <mesh position={[0, -0.88, 0]} geometry={rootGeo}>
        <meshPhysicalMaterial
          color="#d5cdc5"
          roughness={0.25}
          metalness={0.02}
...
function FloatingDiamonds() {
  ...
  const diamondGeo = useMemo(() => {
    const g = new THREE.OctahedronGeometry(0.04, 0);
    return g;
  }, []);
